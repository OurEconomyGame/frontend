...
function attachCEOListeners(compId, token) {
  ...
  document.getElementById('formDividend').addEventListener('submit', async (e) => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.dividend_distributed !== undefined) { showAlert(`Distributed $${data.dividend_distributed} to ${data.shareholders_paid} shareholders!`, 'success'); if (typeof loadCompanyDetails === 'function') loadCompanyDetails(); }
